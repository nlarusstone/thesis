...
\While {$Q$ not empty or $niter == termination$}
	...
	\State $newObj \gets$ incremental($prefix, T, Q, P$) \Comment{Finds possible extensions of prefix (defined below)}
...
\For {$rule \notin prefix$}
	...
	\If {$corr < t$} \Comment{Minimum correct support bound}
		\State continue
	\EndIf
  \State \parbox[t]{.45\linewidth}{$lb = parent.\text{bound(\,)} - parent.\text{minority(\,)} +  \hspace{\algorithmicindent} \dfrac{|cap| - corr}{nsamples} + c$} \Comment{\parbox[t]{.45\linewidth}{Lower bound is equal to parent's lower bound plus the number of mistakes made by the new rule.}}
